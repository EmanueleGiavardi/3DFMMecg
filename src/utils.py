...
def get_R_params(patient_code, beat_idx):
    with open(f'../../R_params/{patient_code}_PARAMS.json', 'r') as f: r_params = json.load(f)
...
def get_R_preproc(patient_code, beat_idx):
    with open(f'../../R_preproc/{patient_code}_PREPROC.json', 'r') as f: r_preproc = json.load(f)
...
def get_df_patient(patient_code):
    dataset_path = "../../datasets/PTB/"
    return pd.read_csv(os.path.join(dataset_path, patient_code+'.csv'))