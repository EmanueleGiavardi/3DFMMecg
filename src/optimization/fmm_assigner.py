...
class FMMAssigner:
    """
    Classe per la gestione del I-Step: assegnazione delle onde ECG.
    """
    ...
    def is_alpha_left(self, alpha1: float, alpha2: float) -> bool:
        """Confronto tra alpha1 e alpha2 in modo circolare (spazio angolare)."""
    ...
    def get_peaks_params(self, alpha: float, beta: float, omega: float) -> Tuple[float, float]:
        """Calcola le posizioni dei picchi (Upper e Lower)."""
    ...
    def get_noisy_set(self, alpha_arr: np.ndarray, omega_arr: np.ndarray) -> np.ndarray:
        """Identifica i componenti che probabilmente rappresentano rumore."""
    ...
    def get_free_set(self, n_waves: int, unused_comp: List[int] = None, free_waves: List[int] = None) -> np.ndarray:
        """Identifica i componenti che non sono stati ancora assegnati."""
    ...
    def assign_r_wave(self, alpha_arr: np.ndarray, omega_arr: np.ndarray, 
        ...
        """Logica per l'identificazione dell'onda R tra i componenti fittati."""
    ...
    def assign_waves(self, parameters: List[Dict], n_obs: int, annotation: float) -> List[Dict]:
        """
        I-Step: Mappa le onde FMM identificate ai specifici componenti ECG (P, Q, R, S, T).
        
        Args:
            parameters: Lista di dizionari contenenti i parametri delle onde fittate.
            n_obs: Numero di osservazioni (campioni).
            annotation: Annotazione del battito ECG (indice temporale del picco R).
            
        Returns:
            Lista di dizionari contenenti i parametri delle onde riordinate in ordine P, Q, R, S, T.
        """
