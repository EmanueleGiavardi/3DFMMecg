...
class LeadProcessor:
    ...
    def __init__(self, lead_data: np.ndarray, lead_idx: int, freq_hz: int):
        ...
        # detrended signal (original signal - low freq trend line)
        self.loess_data = np.array([])

        # local -> related to the specific lead BEFORE the final cleaning (filter artifacts)
        self.local_rpeaks = np.array([])
        self.local_seg = np.array([])
        self.local_rr = np.array([])
        
        # final -> related to the specific lead AFTER the final cleaning, so after that peaks related to each lead gets adjusted according to global peaks
        self.final_rpeaks = np.array([])
        self.final_seg = np.array([])
        self.final_rr = np.array([])

        # tab_codes will store error codes for each beat: [1234 (patientcode???), beat_idx, error_type]
        self.tab_codes = np.array([[1234, 1, 11]])

        # filter1_beats are those peaks who "survived" to the first filtering step (local filtering)
    ...
    def preprocess_single_lead(self):
        ...
        # filter1_beats are those peaks who "survived" to the first filtering step (local filtering)
        filter1_beats, _ = ArtifactFilters.reject_rr_amplitude(self.loess_data, self.local_rpeaks, self.local_rr, self.local_seg, self.freq)
        self.filter1_beats = np.array(filter1_beats)
    ...
    def filter_artifacts(self, global_peaks, global_segments):
        ...
        filter2_beats, _ = ArtifactFilters.reject_excursion(self.loess_data, global_peaks, global_rr, global_segments, global_peaks, self.freq)
        filter3_beats, _ = ArtifactFilters.reject_peak_center(global_peaks, global_rr, global_segments, global_peaks)
        filter5_beats, _ = ArtifactFilters.reject_baseline_trend(self.loess_data, global_peaks, global_rr, global_segments, global_peaks)
        
        if len(filter5_beats) > 0:
            union = np.intersect1d(filter2_beats, np.intersect1d(filter3_beats, filter5_beats))
        else:
            union = np.intersect1d(filter2_beats, filter3_beats)
